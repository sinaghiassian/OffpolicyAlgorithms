...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_of_runs', '-nr', type=int, default=default_params['meta_parameters']['num_of_runs'])
    parser.add_argument('--num_steps', '-ns', type=int, default=default_params['meta_parameters']['num_steps'])
    ...
    save_result(args.save_path, '_stderr_over_runs', np.std(RMSVE_of_runs, axis=0, ddof=1) / np.sqrt(args.num_of_runs), params)
    final_limit = int(0.01 * args.num_steps)
    final_errors = RMSVE_of_runs[:, args.num_steps - final_limit - 1:]
    final_errors_mean_over_steps = np.mean(final_errors, axis=1)
    save_result(args.save_path, '_mean_stderr_final',
                np.array([np.mean(final_errors_mean_over_steps),
                          np.std(final_errors_mean_over_steps, ddof=1) / np.sqrt(args.num_runs)]), params)
    ...
    save_result(args.save_path, '_mean_stderr_auc',
                np.array([np.mean(auc_mean_over_steps),
                          np.std(auc_mean_over_steps, ddof=1) / np.sqrt(args.num_runs)]), params)
